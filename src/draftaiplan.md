...
## CLAUDE.md Configuration `[SKILL ADDED]`

Before starting any work, ensure the following skills are configured in `CLAUDE.md` to run automatically every session:

```
skills:
  - simplify        # Auto-runs on every JS file to enforce DRY, small functions
  - lint-and-test-gate  # Blocks progression if lint or unit tests fail
```

These do not need to be invoked manually — they apply to every file touched.
...
Configure and run all three linters from the start. Do not defer this. The **lint-and-test-gate** skill (configured in CLAUDE.md) enforces this automatically — progression is blocked if any check fails. `[SKILL ADDED]`
...
Apply these principles to every file written. The **simplify** skill (configured in CLAUDE.md) enforces DRY and small-function rules automatically on every JS file. `[SKILL ADDED]`
...
Every JavaScript file must start with a JSDoc file header and every function must have a full JSDoc block with type annotations. The **jsdoc-enforcer** skill applies the exact required template automatically — invoke it on every JS file as it is created. `[SKILL ADDED]`
...
Invoke the **frontend-design** skill before writing any HTML or SCSS. `[SKILL ADDED]` This establishes the visual design system — typography, color palette, animation philosophy — before Claude touches a single line of UI code. Without it, Claude defaults to generic layouts and forgettable animations.
...
Reference `plan/raw-research/accessibility-language-support/research.md`. The **accessibility-auditor** skill is invoked at Step 10 and encodes your specific requirements below — use it instead of running a manual Lighthouse check. `[SKILL ADDED]`
...
Playwright tests run against the actual server (`npm run dev`) and the client served from `client/`. They test the full stack through the browser. Add the **Playwright MCP server** to Claude Code so Claude can run and debug these tests directly rather than just writing them. `[SKILL ADDED]`
...
At each step: write the code, run the relevant linter (`npm run lint`), write or run the relevant tests (`npm run test:unit`), and fix any issues before moving on. The **lint-and-test-gate** and **simplify** skills run automatically throughout via CLAUDE.md. `[SKILL ADDED]`

1. `server/game/paytable.js` — define symbols, weights, and payout rules; write `paytable.test.js` alongside it. Invoke `/jsdoc-enforcer` after the file is written. `[SKILL ADDED]`
2. `server/game/rng.js` — weighted draw function; write `rng.test.js` alongside it. Invoke `/jsdoc-enforcer` after the file is written. `[SKILL ADDED]`
3. `server/game/game.js` — spin orchestration and payout calculation; write `game.test.js` and the simulation test alongside it. Invoke `/jsdoc-enforcer` after the file is written. `[SKILL ADDED]`
4. **Confirm all unit tests pass and simulated RTP is on target before writing any routes.** Invoke `/game-math-validator` here — it cross-checks your paytable against the RTP target (92–95%), hit frequency range (25–35%), and volatility profile. Do not proceed to step 5 until it signs off. `[SKILL ADDED]`
5. `server/server.js` + `server/routes/spin.js` — Express app and `POST /api/spin` endpoint; write `spin.route.test.js` alongside it. Invoke `/jsdoc-enforcer` on both files. `[SKILL ADDED]`
6. `client/index.html` + SCSS partials — **invoke `/frontend-design` before writing any markup or styles.** Then build the static layout, mobile-first; run `npm run watch:css` and `npm run lint:html` / `npm run lint:scss` as you go. `[SKILL ADDED]`
7. `client/js/api.js` — fetch wrapper pointing at the configurable server base URL. Invoke `/jsdoc-enforcer` after the file is written. `[SKILL ADDED]`
8. `client/js/ui.js` — wire up DOM, animations, credit display using responses from `api.js`; add Playwright e2e tests for spin flow and credit updates. Invoke `/jsdoc-enforcer` after the file is written. `[SKILL ADDED]`
9. `client/js/audio.js` — sounds and mute toggle; add Playwright e2e test for mute persistence. Invoke `/jsdoc-enforcer` after the file is written. `[SKILL ADDED]`
10. **Accessibility pass** — invoke `/accessibility-auditor` to check ARIA labels, keyboard nav, and Lighthouse 90+ target against your specific requirements. Add Playwright keyboard-nav test. `[SKILL ADDED]`
11. **Final gate** — run `npm run lint && npm run test`. The **lint-and-test-gate** skill enforces this as a hard block. Everything must be green before the project is considered done. `[SKILL ADDED]`
