...
import { STARTING_CREDITS, MIN_BET, MAX_BET, REEL_COUNT } from '../../server/game/game.js';
...
describe('POST /api/spin', () => {
  it('returns 200 with a 5×3 reels array and metadata', async () => {
    const { status, body } = await postSpin({ bet: 10, credits: STARTING_CREDITS });
    assert.strictEqual(status, 200);
    assert.ok(Array.isArray(body.reels));
    assert.strictEqual(body.reels.length, REEL_COUNT);
    for (const col of body.reels) {
      assert.ok(Array.isArray(col));
      assert.strictEqual(col.length, 3);
    }
    assert.ok(Array.isArray(body.payline));
    assert.strictEqual(body.payline.length, REEL_COUNT);
  ...
  it('credits decrease by bet on a non-winning spin', async () => {
